...
public class MatchDataGeomStatsGameElementsL1 extends MatchDataView implements IMatchDataView {

    public MatchDataGeomStatsGameElementsL1(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void calculateFromDocuments(List<Document> documents) {
        ...
        for (Document document : documents) {
            ...
            for (Map<String, Object> cycle : cycles) {      // throwing null pointer exception
                if ((boolean) cycle.get(CycleModel.CS_CARGO_SCORE_KEY) | (boolean) cycle.get(CycleModel.CS_HATCH_SCORE_KEY) |
                        (boolean) cycle.get(CycleModel.RO_CARGO_SCORE_L1_KEY) | (boolean) cycle.get(CycleModel.RO_HATCH_SCORE_L1_KEY)) {
        ...
        if (!didSomething) {
            ...
            setValueText(formatNumberAsString(geometricMean)+ "   " +
                    ...
                    formatPercentageAsString(geometricCoeffVar), "gray");
        }
    }
}
