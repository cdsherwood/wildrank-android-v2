...
public class MatchDataGeomStatsGameCargoAll extends MatchDataView implements IMatchDataView {

    public MatchDataGeomStatsGameCargoAll(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void calculateFromDocuments(List<Document> documents) {
        ...
        for (Document document : documents) {
            ...
            for (Map<String, Object> cycle : cycles) {      // throwing null pointer exception
                if ((boolean) cycle.get(CycleModel.CS_CARGO_SCORE_KEY) | (boolean) cycle.get(CycleModel.RO_CARGO_SCORE_L1_KEY)
                        | (boolean) cycle.get(CycleModel.RO_CARGO_SCORE_L2_KEY) | (boolean) cycle.get(CycleModel.RO_CARGO_SCORE_L3_KEY)) {
