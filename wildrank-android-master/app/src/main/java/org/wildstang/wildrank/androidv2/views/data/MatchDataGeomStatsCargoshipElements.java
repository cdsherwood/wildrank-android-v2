...
public class MatchDataGeomStatsCargoshipElements extends MatchDataView implements IMatchDataView {

    public MatchDataGeomStatsCargoshipElements(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void calculateFromDocuments(List<Document> documents) {
        ...
        for (Document document : documents) {
            ...
            for (Map<String, Object> cycle : cycles) {      // throwing null pointer exception
                if ((boolean) cycle.get(CycleModel.CS_CARGO_SCORE_KEY)) {
            ...
            for (Map<String, Object> cycle : cycles) {      // throwing null pointer exception
                if ((boolean) cycle.get(CycleModel.CS_HATCH_SCORE_KEY)) {
        ...
        if (!didSomething) {
            ...
            Log.d("wildrank_geoMean_CS_Car", formatNumberAsString(geometricMeanCargo));
            Log.d("wildrank_geoMean_CS_Hat", formatNumberAsString(geometricMeanCargo));
